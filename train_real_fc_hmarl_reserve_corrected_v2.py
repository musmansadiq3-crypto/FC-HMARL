...
SMOKE_OUTPUT_DIRECTORY = (
    ...
    / "outputs"
    / "results"
    / "real_fc_hmarl_reserve_corrected_smoke"
)
...
def main():
    ...
    section(
        "FC-HMARL STEP 7R-F - RESERVE-CORRECTED REAL-DATA TRAINING"
    )
    ...
    if args.smoke_test:
        ...
        summary_file = (
            output_directory
            / "real_fc_hmarl_reserve_corrected_smoke_summary.json"
        )
        ...
        summary_file = (
            output_directory
            / "real_fc_hmarl_reserve_corrected_training_summary.json"
        )


    # ========================================================
    # STEP 7R-F CLEAN-RUN PROTECTION
    # ========================================================
    # Old checkpoints were trained with the pre-correction reserve
    # economics and MUST NOT be resumed into this experiment.
    if args.resume_episode is not None:

        raise ValueError(
            "Step 7R-F must begin from fresh SAC weights and replay "
            "buffers. Do not use --resume-episode for the initial "
            "reserve-corrected training run."
        )

    if not args.smoke_test:

        existing_corrected_artifacts = []

        if checkpoint_directory.exists():
            existing_corrected_artifacts.extend(
                path
        ...
        if existing_corrected_artifacts:

            preview = "\n".join(
                f"  {path}"
                for path in existing_corrected_artifacts[:10]
            )

            raise FileExistsError(
                "Reserve-corrected output/checkpoint folders already "
                "contain files. Refusing to overwrite a prior corrected "
                "experiment. Move/archive those files first if you "
                "intentionally want a brand-new run.\n"
                + preview
            )
    ...
    # ========================================================
    # CLEAN CORRECTED TRAINING START
    # ========================================================
    ...
    section(
        "STEP 7R-F RESERVE-CORRECTED TRAINING COMPLETE"
    )
    ...
    print(
        "[OK] No pre-correction checkpoint was reused."
    )
