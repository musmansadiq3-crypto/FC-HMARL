...
SUMMARY_FILE = (
    OUTPUT_DIR
    / "real_rl_validation_summary.csv"
)

METHOD_FILE = (
    OUTPUT_DIR
    / "real_rl_validation_methods.csv"
)

METADATA_FILE = (
    OUTPUT_DIR
    / "real_rl_validation_metadata.json"
)
...
section(
    "FC-HMARL STEP 7L-B - LOADING VALIDATION DATA"
)
...
X_val = np.asarray(
    sequence_archive["X_val"],
    dtype=np.float32,
)

y_val = np.asarray(
    sequence_archive["y_val"],
    dtype=np.float32,
)

idx_val = np.asarray(
    sequence_archive["idx_val"]
)
...
section(
    "LOADING TRAINING-ONLY SCALER FOR VALIDATION"
)
...
section(
    "BUILDING VALIDATION-SEASONAL FORECASTS"
)


# X_val contains the preceding 168 hours.
#
...
section(
    "BUILDING CURRENT PHYSICAL VALIDATION VALUES"
)
...
print(
    "  y_val[:, 0, :] for current physical realization"
)
...
print(
    "Validation archive does NOT load:"
)
...
# Source archive contains all chronological splits, but this script
# loads only X_val/y_val/idx_val as working RL validation arrays.
...
print(
    "[OK] TRAIN and TEST arrays exist in the source archive "
    "but are not loaded into RL validation variables."
)
...
print(
    "[OK] RL validation archive uses VALIDATION split."
)
...
print(
    ...
    "to construct RL validation samples."
)
...
print(
    "[OK] Real-data FC-HMARL validation archive is ready."
)
